...
## 0. Ежедневная автоматизация — рабочий flow

Запуск считается успешным **только когда новый пост уже опубликован на
production `opten.space`** (RU + EN отдают 200). Без живой публикации работа
не закрыта.

Архитектура:
- Cron в Codex Desktop запускает `daily-opten-blog-post` раз в день.
- Scheduled Codex **не имеет надёжного shell-доступа к `github.com:443`** и
  может не дать создать `.git/index.lock` даже внутри подготовленного workspace.
  Даже если в `~/.codex/config.toml` есть `[sandbox_workspace_write]
  network_access = true`, session logs могут стартовать с `network_access:false`.
  Поэтому внутри scheduled Codex нельзя делать `git clone
  https://github.com/...`, `git fetch`, `git pull`, `git push`, `git add`, `git
  commit` или `gh`.
- Codex пишет пост в локальном workspace из cache-clone, запускает build/verify и
  ставит publish-marker. `git add`, `git commit` и сетевой push прямо в
  `origin/main` делает Windows Scheduled Task `CodexDailyOptenBlogMainPublisher`
  вне Codex sandbox.
- Без веток и без PR: publisher пушит готовый commit прямо в `main`, при
  необходимости предварительно делает `rebase origin/main`.
- Основной checkout `C:\Projects\opten-website` не трогаем вообще — там может
  быть пользовательский WIP.
- Lock через `preflight.ps1 acquire`/`release` защищает от двойного запуска.

### 0.1. Порядок шагов

1. **Lock.** `preflight.ps1 acquire`. Exit 75 → другой запуск активен, выйти со
   `skipped`. Другой ненулевой → hard-блокер, выйти `failed`. Lock освобождать
   всегда в конце (даже при skipped/failed/blocked).
2. **Workspace из cache.** После выбора slug создать workspace только через
   helper:
   ```powershell
   powershell -ExecutionPolicy Bypass -File C:\Users\КОМП\.codex\automations\daily-opten-blog-post\preflight.ps1 prepare-workspace -Slug <slug>
   ```
   Рабочая папка:
   `C:\Users\КОМП\.codex\automations\daily-opten-blog-post\runs\<slug>`.
   Helper клонирует из
   `C:\Users\КОМП\.codex\automations\daily-opten-blog-post\repo-cache\opten-website`,
   который обновляет publisher вне sandbox. В основной checkout не лезть.
3. **Выбор темы.** Кандидаты — §7. Использованные slug'и — `src/content/blog/index.ts`
   в workspace. Первый slug из §7, которого нет в index.ts. Если все §7 опубликованы —
   release lock, статус `no-topics`.
4. **Контент + изображения + интеграции** по §2–§6 ниже.
5. **Build.** `npm ci` (если `node_modules` нет) → `npm run build`. Проверить
   prerendered RU/EN HTML: `lang`, canonical, reciprocal hreflang,
   `BlogPosting`/`HowTo`/`FAQPage`, image refs, размеры всех jpg = 1600x900.
   Build упал — release lock, статус `failed` с логом.
6. **Mark ready без git commit.** После успешного build оставить изменения в
   workspace и не запускать `git add`/`git commit` из scheduled Codex. `mark-ready`
   проверит, что изменены только файлы одного blog post scope:
   `src/content/blog/<slug>.ts`, `src/content/blog/index.ts`,
   `scripts/seo-routes.ts`, `scripts/sitemap.mjs`, `scripts/llms.mjs`,
   `src/i18n/paths.ts`, `public/blog/<slug>/`. Model registry/summary файлы
   должны оставаться read-only reference.
   ```powershell
   powershell -ExecutionPolicy Bypass -File C:\Users\КОМП\.codex\automations\daily-opten-blog-post\preflight.ps1 mark-ready -Slug <slug> -Title "blog: <slug>"
   powershell -ExecutionPolicy Bypass -File C:\Users\КОМП\.codex\automations\daily-opten-blog-post\preflight.ps1 run-publisher
   ```
7. **Publisher.** Publisher делает surgical `git add`, `git commit`, `fetch`,
   `rebase` при необходимости, `git push origin HEAD:main`, ждёт production URL
   и удаляет workspace. Если publisher ещё работает, финальный статус scheduled
   Codex: `queued-for-local-publisher`.
8. **Release lock.** `preflight.ps1 release`.
9. **Финальная проверка.** Если publisher успел завершить, он сам проверяет:
   `https://opten.space/blog/<slug>` и `https://opten.space/en/blog/<slug>` должны
   ответить 200. Если не успел, это не ошибка генерации: смотреть
   `C:\Users\КОМП\.codex\automations\daily-opten-blog-post\logs\publisher.log`.

Не использовать: `.codex-blog-worktrees/`, `.codex-blog-workspaces/`, `git
worktree add`, GitHub Connector как publish fallback. Connector может читать
repo, но ранее возвращал `403 Resource not accessible by integration` при
создании branch.
...
- **Эталон стиля — готовые генерации в `C:\Projects\opten-website\seo\Reference`**
...
Эталонные референсы лежат в `C:\Projects\opten-website\seo\Reference` — их
