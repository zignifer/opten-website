...
assert.match(content, /PRIVATE_COURSE_PRICE_RUB\s*=\s*9990/, "Private course base RUB price must be 9 990");
assert.match(content, /PRIVATE_COURSE_LIST_PRICE_RUB\s*=\s*9990/, "Private course list RUB price must stay 9 990");
assert.match(content, /PRIVATE_COURSE_PRICE_USD\s*=\s*139/, "Private course sale price must be 139 USD");
assert.match(content, /PRIVATE_COURSE_LIST_PRICE_USD\s*=\s*139/, "Private course list price must be 139 USD");
...
assert.doesNotMatch(tokenApi, /telegram-course-preview|hasTelegramCoursePreviewAccess|telegramPreviewClaim/, "Token API must not authorize playback through Telegram claims");
