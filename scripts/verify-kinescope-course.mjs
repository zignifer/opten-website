...
assert.match(content, /PRIVATE_COURSE_PRICE_USD\s*=\s*69/, "Private course base price must be 69 USD");
assert.match(content, /PRIVATE_COURSE_LIST_PRICE_USD\s*=\s*69/, "Private course list price must stay 69 USD");
...
assert.doesNotMatch(content, /privateCourseHiddenIntroLesson|HIDDEN_INTRO_SLUG|AI контент-завод/, "Retired lesson zero must stay out of the course content");
...
assert.match(content, /PRIVATE_COURSE_TITLE_RU\s*=\s*["']Курс по ИИ["']/, "Private course RU title must match the canonical product name");
assert.match(content, /PRIVATE_COURSE_TITLE_EN\s*=\s*["']AI Course["']/, "Private course EN title must match the localized product name");
...
assert.doesNotMatch(tokenApi, /telegram-course-preview|hasTelegramCoursePreviewAccess|telegramPreviewClaim|telegram-hidden-intro|discountClaimToken/, "Token API must not authorize playback through Telegram claims");
...
assert.doesNotMatch(serverCourse, /hidden-intro|a4722357-b131-491f-8ca0-cdd11d927630/, "Retired lesson zero must stay out of the Kinescope whitelist");
