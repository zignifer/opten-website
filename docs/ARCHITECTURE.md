...
│                                                                              │
│   Routes (18 client + catch-all 404; 18 prerendered HTML files):             │
...
│         download-skill                                                       │
│     EN siblings (9 prerendered):                                             │
│       /en/, /en/pay, /en/welcome, /en/about, /en/blog, /en/blog/:slug,       │
│       /en/privacy, /en/terms, /en/refund                                     │
│     Catch-all: <Route path="*" element={<NotFound />}> — runtime noindex     │
│                                                                              │
│   Postbuild prerender produces 18 dist/<route>/index.html files with         │
...
**EN siblings** (9 prerendered, identical components — language flips via `LangProvider`):
`/en/`, `/en/pay`, `/en/welcome`, `/en/about`, `/en/blog`, `/en/blog/:slug`, `/en/privacy`, `/en/terms`, `/en/refund`.
Source of truth: [`EN_SIBLINGS`](../../src/i18n/paths.ts) in `src/i18n/paths.ts` — must mirror the EN entries in [`scripts/seo-routes.ts`](../../scripts/seo-routes.ts).
...
**Bilingual URLs (post-v1.0 state):**
- **18 prerendered HTML files**: 9 RU canonical (`/`, `/pay`, `/welcome`, `/about`, `/blog`, `/blog/gpt-image-2`, `/privacy`, `/terms`, `/refund`) and 9 EN siblings (mirror paths under `/en/`).
