...
- **Functions** use Vercel's Node.js handler signature `(req, res)`:
  ...
  - [`api/prompt-workbench.ts`](../../api/prompt-workbench.ts) — authenticated landing Improve endpoint. It accepts only `improve`, enforces 20–6,000-character prompt bounds, optional compressed references, the extension popup's 14 image/video profiles, and the site-only `codex | claude | gemini` Vibe Coding profiles. A website Bearer JWT is locally verified (or checked through the canonical GoTrue `/auth/v1/user` endpoint in local development without `SUPABASE_JWT_SECRET`) before the function calls promptscore-proxy Claude Haiku and the shared operation ledger. Image/video uses popup-equivalent `rewriter.md` + `source: popup`; Vibe Coding uses site-owned `vibecoding-cleaner.md`, hidden `_coding-*` proxy adapters, `source: website_vibecoding`, explicit-image-reference gating, and a timestamp/model/prompt-bound server HMAC. The proxy rejects unsigned internal-mode requests before reservation and releases the exact reservation when its own finalizer sees a no-op or unsafe result; the site maps `422 no_improvement` to a refund message without retrying. Anonymous requests are rejected before proxy use; signed-in Free users spend one-time signup credits until exhausted, and Pro users spend the normal monthly 300-operation cycle. In local Vite development, `vite.config.ts` mounts this same handler at `/api/prompt-workbench`.
