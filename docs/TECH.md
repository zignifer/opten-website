...
- **Functions** use Vercel's Node.js handler signature `(req, res)`:
  ...
  - [`api/prompt-workbench.ts`](../../api/prompt-workbench.ts) — Pro-only landing quick-Improve endpoint. It accepts only `improve`, enforces the extension popup's exact 14-model list, 20–6,000-character prompt bounds, and optional compressed references. A website Bearer JWT is locally verified (or checked through the canonical GoTrue `/auth/v1/user` endpoint in local development without `SUPABASE_JWT_SECRET`) and checked for a live Pro subscription before the function forwards the popup-equivalent request (`rewriter.md`, Anthropic image blocks, `max_tokens: 1200`, `count_usage: true`, `source: popup`) to promptscore-proxy Claude Haiku and the shared operation ledger. Anonymous and signed-in Free requests are rejected before proxy use. In local Vite development, `vite.config.ts` mounts this same handler at `/api/prompt-workbench`.
