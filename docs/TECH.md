...
> Snapshot taken 2026-05-14, refreshed 2026-07-18 (public Learn, Learn Finds,
> launched Kinescope course, Telegram discount-only course offer, course API endpoints,
> route count update).
...
- **Website auth/session helpers** live in [`src/lib/optenAuth.ts`](../../src/lib/optenAuth.ts) and store the website Supabase session in `localStorage.opten_space_session_v1`. `/pay`, `/account`, `/app/*`, and paid course access prefer website JWTs; `/pay` and `/account` retain extension JWT fallback for shipped flows.
...
- **JWT verification (Phase 88 dual-issuer):** site serverless functions verify user JWTs **locally** with `jose` against the Supabase issuer allowlist and `SUPABASE_JWT_SECRET`; no `/auth/v1/user` session lookup. `api/download-skill.ts` gates Pro skill ZIP by `subscriptions`. `api/kinescope-course-token.ts` and `api/course-prompt.ts` gate paid course content by `course-access-summary`.
...
- **Telegram course offer** is also extension-owned: the legacy-named `telegram-hidden-intro-webhook` stores started users and events. `/start` directly creates or reuses one 24h 20% `course_discount_claims` token without a navigation, free-lesson, or lesson-zero branch. A new active claim first sends the Figma-approved HTML offer with one course button and the public Telegram channel link in the text, then schedules the source-controlled 720p H.264/AAC intro video from `/assets/telegram/ai-content-marketing-2026-intro-v2.mp4` (5.9 MB) as a background task; a repeated request returns the same claim without refreshing it or replaying the video. Retired lesson callbacks enter this same direct course flow only for compatibility with old buttons. The token only discounts checkout and never authorizes playback, private prompts, or Opten generator links; all 16 lessons require the course entitlement. Every locked lesson uses the normal purchase UI with no Telegram unlock CTA. The collection-level generator block is visible before materials on every paid lesson page, but absent from the course root: course buyers and active Pro users get the working links without the sales description, while everyone else sees locked previews with one compact `/pay` CTA. `telegram-hidden-intro-opened` is a fail-closed compatibility endpoint that returns HTTP 410; `telegram-hidden-intro-stats`, `telegram-hidden-intro-broadcast`, `telegram-hidden-intro-broadcasts`, `telegram-hidden-intro-assets`, and `telegram-hidden-intro-reminders` are service endpoints protected by `X-Opten-Admin-Secret` for mutations. The website `/admin` surface calls them only through server-side `/api/admin/*` proxies after website JWT + owner allowlist checks. Admin v1 ships stats through `/api/admin/telegram-stats`, guarded broadcast send through `/api/admin/telegram-broadcast`, broadcast history/delete through `/api/admin/telegram-broadcasts`, and image upload through `/api/admin/telegram-upload-photo`; the broadcast proxy repeats a dry-run before final send and rejects stale recipient counts. Non-dry-run broadcasts are deletable only when the extension backend has stored Telegram `message_id` values for recipients. Uploaded broadcast photos are compressed client-side, stored in private `telegram_broadcast_assets`, and served publicly only by random token URL for Telegram `sendPhoto`.
...
- `EN_SIBLINGS` constant lives in [`src/i18n/paths.ts`](../../src/i18n/paths.ts) — must stay in sync with EN entries in `scripts/seo-routes.ts` for marketing, blog, models, public Learn lessons, and Learn Finds. Paid course/template/app routes intentionally do not get EN siblings for SEO under the current routing policy.
...
- **Functions** use Vercel's Node.js handler signature `(req, res)`:
  ...
  - [`api/prompt-workbench.ts`](../../api/prompt-workbench.ts) — authenticated landing Improve endpoint. It accepts only `improve`, enforces 20–6,000-character prompt bounds, optional compressed references, the extension popup's 14 image/video profiles, and the site-only `codex | claude | gemini` Vibe Coding profiles. A website Bearer JWT is locally verified (or checked through the canonical GoTrue `/auth/v1/user` endpoint in local development without `SUPABASE_JWT_SECRET`) before the function calls promptscore-proxy Claude Haiku and the shared operation ledger. The UI stores only the selected type/model in per-tab `sessionStorage`; prompt content remains memory-only. Explicit coding-workflow text paired with an Image/Video model returns `409 vibecoding_mode_required` after auth but before proxy use, while the UI switches to Vibe Coding and asks for a second click without spending usage. Image/video otherwise uses popup-equivalent `rewriter.md` + `source: popup`; Vibe Coding uses site-owned `vibecoding-cleaner.md`, hidden `_coding-*` proxy adapters, `source: website_vibecoding`, explicit-image-reference gating, and a timestamp/model/prompt-bound server HMAC. The proxy rejects unsigned internal-mode requests before reservation. After Anthropic returns, Vibe finalization must run before any HTTP 200 serialization. A rejected first draft gets at most one corrective provider call inside the same reservation, with no second `checkUsage` or credit. If that result also fails for no-op, introduced meta response, excessive compression, missing semantic anchors, or another guardrail, the exact reservation is released. The site maps `422 no_improvement` to a refund message without retrying. A handler-level proxy test exercises this order and proves one reservation. Anonymous requests are rejected before proxy use; signed-in Free users spend one-time signup credits until exhausted, and Pro users spend the normal 300-operation monthly cycle. In local Vite development, `vite.config.ts` mounts this same handler at `/api/prompt-workbench`.
  - [`api/kinescope-course-token.ts`](../../api/kinescope-course-token.ts) — checks paid `course-access-summary` or server-validates a Telegram claim for lessons 1–3, signs a short-lived Kinescope `drmauthtoken`, and returns an embed URL.
  ...
  - [`api/admin/telegram-upload-photo.ts`](../../api/admin/telegram-upload-photo.ts) — validates website JWT + owner allowlist and uploads compressed broadcast photos by proxying `telegram-hidden-intro-assets` with `TELEGRAM_ADMIN_SECRET`.
...
- Paid course downloadable materials under `public/assets/space/courses/**` are static public assets; locked prompt bodies stay server-side in `api/_shared/coursePromptBodies.ts`.
...
| `TELEGRAM_ADMIN_SECRET` | Server-only secret used by website admin proxy to call Telegram service Edge Functions | `api/admin/telegram-stats.ts`, `api/admin/telegram-broadcast.ts`, `api/admin/telegram-broadcasts.ts`, `api/admin/telegram-upload-photo.ts` |
...
- **Launched paid course content**: [`src/content/space/privateCourse.ts`](../../src/content/space/privateCourse.ts), [`privateCourseExtras.ts`](../../src/content/space/privateCourseExtras.ts), [`api/_shared/kinescopeCourse.ts`](../../api/_shared/kinescopeCourse.ts), and [`api/_shared/coursePromptBodies.ts`](../../api/_shared/coursePromptBodies.ts). The course is publicly promoted, while its SPA routes remain noindex and excluded from sitemap/llms under the current routing policy. Promo-code pricing is not content; it is extension-owned backend data in `course_promo_codes`.
