...
8. **SEO2 blog posts have their own blocking gate.** Before commit, run
...
- Keep SEO keywords grammatical in visible copy. Do not paste raw query strings into titles, descriptions, intros, step bodies, FAQ, or examples just to preserve exact search wording: avoid phrases like `бизнес портрет нейросеть`, `деловой портрет нейросеть`, `Апскейл фото нейросеть`, `озвучка видео нейросеть тг`, `ИИ инфлюенсер это что`, `kling 3.0 цена`. Rewrite them as natural prose: `бизнес-портрет через нейросеть`, `деловой портрет в нейросети`, `апскейл фото через нейросеть`, `нейросетевая озвучка видео для Telegram`, `что такое ИИ-инфлюенсер`, `цена Kling 3.0`. Exact query strings are allowed only when the sentence explicitly talks about a search query and still reads like normal Russian.
- Each `step.title` is the H2 / table-of-contents anchor — make it scannable. The `step.body` is the explanation.
- New SEO2 posts must leave the optional `before`/`after` step fields unused and must not create visible «Было / Стало», `Before / After`, or equivalent paired comparison blocks. Present a standalone final example, checklist, sequence, annotated result, or decision breakdown instead. Existing published posts are grandfathered and remain unchanged.
...
- **SEO2 inline images are mini-explainers, not title cards.** Follow the `ai-headshot-generator` / `ai-ugc-for-brands` reference pattern from `seo2/Reference/`: one large hero label plus 3-4 generated-in-image supporting labels, checklist rows, micro-cards, timeline nodes, or anatomy callouts. Do not use «Было / Стало», `Before / After`, or equivalent paired comparison labels. A dark photo with one big headline and decorative icons is not enough.
- **SEO2 art starts from a production sheet, not a vague image hint.** Before generating, create `seo2/visual-plans/<slug>.md` with one concrete cover concept and separate RU/EN inline frame concepts. Keep visual instructions out of `topic-registry.json`. Each post needs a distinct subject/scene family; do not reuse the same dark desk + floating UI panels + lime connector network motif across articles.
- **Human presence is deliberate and limited.** A standard SEO2 inline series declares `Humanized frames: 1`; each frame declares `Human presence: none | hands-only | human-detail | single-person | interaction`, and every humanized frame explains its `Human purpose`. Use a working hand, meaningful gesture, one protagonist, or relevant interaction to prove action, decision, scale, review, or handoff — never generic stock-office decoration. A second humanized frame is allowed only for an inherently human-centered topic with a written `Human-heavy exception`; at least one inline frame remains human-free, and the cover does not satisfy the quota. Prompt human-free frames with `no people, no hands, no human silhouettes`. Preserve the same person, pose, action, clothing, and crop in the RU/EN localization pair.
- **Blog promo banners are reusable CTA assets, not SEO2 inline explainers.** Save them under `public/blog/_banners/` as wide, shallow generated rasters with a clean left-side negative-space zone and a right-side Opten-style visual. The banner headline, description, and button are rendered in HTML by the blog page, so do not bake CTA text into the image. For a new SEO2 article, point the primary promo CTA to the selected topic's `primaryDestination`: `/`, `/prompt-library`, `/learn`, or `/learn/courses/ai-content-marketing-2026`. The destination must match the article intent; contextual secondary links may support another Opten surface without replacing the primary CTA.
...
- [ ] SEO2 visual plan declares one humanized inline frame by default, a `Human presence` value for every frame, and a semantic `Human purpose` for the humanized frame; no series uses people in every inline frame.
