...
site-only `Вайб-кодинг` / `Vibe coding` type with Codex, Claude, or Gemini,
enter a prompt, attach up to 8 ephemeral image references, and invoke improvement.
The selected type/model preference is restored from per-tab `sessionStorage` after
a reload; prompt text and references remain memory-only. If an Image/Video selection
receives a request with explicit coding-workflow markers such as Vibe Coding,
Super Powers plus implementation stages, branch Main workflow, or commit/push, the
client switches to `Вайб-кодинг / Codex` and requires a confirming second click.
The server independently returns `409 vibecoding_mode_required` after auth but
before proxy use, so a stale/bypassed client cannot spend a credit in the wrong mode.
...
Plan Mode, or any other unstated requirement. Explicit user instructions to plan,
  test, verify, use a named tool/skill, stay on a branch, preserve context, commit,
  push, or work in stages remain separate requirements in the cleaned request. When
  the input is feedback about a previous rewrite or cleaner result, convert its
  explicit complaints and requested investigation into direct imperative instructions
  instead of answering conversationally. Long
  requests retain their level of detail instead of becoming a one-line summary. It
  preserves the user's prose language and URLs, paths, commands, versions, quoted
...
| content (private) | `C:\Projects\content` | Canonical content-production rules, style sources, aggregate metrics, and Graphify map |
...
- The landing `PromptWorkbench` is visible publicly but its AI action requires a signed-in website session and is otherwise ephemeral: prompt text, AI results, selected files, compressed reference bytes, and preview URLs stay in component memory and are not written to `localStorage`. Only the selected workbench type/model is kept in per-tab `sessionStorage` under `opten_prompt_workbench_preferences_v1`, so a deploy or reload does not silently reset Vibe Coding back to Image. The browser calls only same-origin `/api/prompt-workbench`; logged-out clicks show an authentication error, signed-in Free clicks may spend one-time signup credits, exhausted Free users receive the proxy limit error, and live Pro sessions spend the normal 300-operation monthly cycle. The server verifies the JWT and 20-character minimum, allows exactly 14 popup-compatible image/video profiles plus site-only `codex | claude | gemini`, and calls the existing `promptscore-proxy` Claude Haiku rewrite endpoint with legacy-compatible `count_usage: true`; the proxy is the final usage gate and ignores client billing fields through atomic reserve-before-provider accounting. Explicit coding-workflow text submitted with an Image/Video model returns `409 vibecoding_mode_required` before proxy use; the UI switches to `Вайб-кодинг / Codex`, asks for a confirming second click, and states that no credit was used. Image/video otherwise keeps the popup rewriter contract. Vibe Coding uses its own site-only cleaner and hidden proxy adapters; the website HMAC-signs the original request/model, and proxy runtime guardrails must finalize the provider response before the handler can serialize HTTP 200. An exact no-op, meta response, excessively compressed result, missing semantic anchor, or other rejected first Vibe draft gets at most one corrective Anthropic attempt inside the same usage reservation, with no second `checkUsage` and no second credit. If the corrective result is still rejected, the proxy returns `422 no_improvement`, releases the exact reservation, refreshes the visible account balance, and never silently replaces a billed result with the original prompt. The website itself does not retry. A handler-level test must execute this ordering and prove the reservation is created once; a unit test of the finalizer alone does not protect billing. Uploaded references are capped at 8 files, 10 MB per source, compressed client-side to JPEG with a 512 px maximum long edge, and must be revoked/cleared when removed or unmounted; Vibe Coding forwards them only when the prompt explicitly references an image. During `npm run dev`, `vite.config.ts` mounts the same Node handler as dev-only middleware so localhost exercises the production handler contract. The reference-strip add button uses an inset focus ring: the strip scrolls horizontally, so an external ring would be clipped by its overflow boundary after the native file picker closes. Copying is local and does not consume usage: desktop shows a secondary `Скопировать` action beside `Улучшить`, while mobile shows the extension-matching outline copy icon in the workbench header. Both copy the current prompt and cross-fade to the extension's filled icon for 500 ms after success.
...
8. Deliberately humanize new SEO2 inline series without turning every frame into a people shot. The production sheet must declare `- Humanized frames: 1` by default and every inline frame must declare `- Human presence: none | hands-only | human-detail | single-person | interaction`. The humanized frame must also declare `- Human purpose:` and use hands, a partial human detail, one protagonist, or a meaningful interaction to prove an action, decision, scale, review, or handoff from the adjacent article step; generic stock-office people are not semantic proof. A second humanized inline frame is allowed only for an inherently human-centered topic and requires a non-empty `- Human-heavy exception:` reason; at least one inline frame must remain human-free. The cover does not satisfy the inline quota. For `none`, explicitly prompt `no people, no hands, no human silhouettes`; for localized RU/EN pairs, preserve the same person, pose, action, crop, and scene while changing only the text.
9. Blog promo banners are the exception to the inline-text rule: they are reusable generated rasters with a clean left-side negative-space zone and a right-side Opten-style visual; the heading, description, and CTA button are rendered as accessible HTML so the same asset can be reused and localized. Save them under `public/blog/_banners/` and use dark SaaS editorial styling (`#011417` + exact `#9CFB51`). For new SEO2 posts, the promo CTA must follow the selected topic's `primaryDestination` in `seo2/topic-registry.json`: `/` for the prompt generator/Pro surface, `/prompt-library` for the library, `/learn` for the public learning portal, or `/learn/courses/ai-content-marketing-2026` for the standalone course. Bounded research batches should stay approximately 50/50 between product destinations (`/`, `/prompt-library`) and learning destinations (`/learn`, course), with near-even distribution inside each side; do not force an unrelated product into an article.
10. The single SEO topic registry must pass `npm run verify:seo-topics` before article generation. The gate checks schema, statuses, keyword evidence, RU/EN coverage, deduplication, and parity with published blog slugs. `npm run build` runs the same gate first.
11. Every `<img>` gets explicit `width`/`height` (CLS guard).
12. `<html lang>` is baked at prerender, NEVER mutated at runtime (Phase 3 D-06).
13. Locale-neutral slugs — `/blog/foo` is the same slug in RU and EN.
14. `npm run build` must pass locally — the sitemap + llms floor checks fail loudly when routes are forgotten.
15. User command `напиши SEO-статью` means: run `npm run start:seo` first. Legacy aliases `start SEO`, `start seo`, and `старт SEO` mean the same thing. If it prints `start-seo: research-needed`, follow `seo2/TOPIC-PIPELINE.md`, append a bounded reviewed set to the single `seo2/topic-registry.json`, validate it, rerun `start:seo`, and continue with exactly one post. If it prints `start-seo: next-topic`, create exactly one SEO2 post for that slug using `seo2/blog-post-instruction.md`.
16. New SEO2 blog posts must pass `npm run verify:seo2-blog -- <slug>`
    ...
    images, generated image files present, promo CTA matching the topic's
    `primaryDestination`) and basic content constraints.
...
17. New SEO2 posts must not use the paired `before`/`after` step fields, visible
