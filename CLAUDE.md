...
The same `/admin` shell has a top-level `Админка / Контент-машина` switch.
`Контент-машина` is a read-only knowledge reader, not a ninth operational admin
tab. Its source of truth is the private `zignifer/content` repository at
`C:\Projects\content`; the content repo generates
`exports/admin/content-machine.json`, then
`npm run sync:content-machine -- --source C:\Projects\content\exports\admin\content-machine.json`
copies a validated, bounded snapshot into a server-only TypeScript module in
this repo. The browser may receive that snapshot only through
`GET /api/admin/content-machine` after the normal website JWT and owner
allowlist have passed. Never import the raw snapshot into the public SPA,
fetch private GitHub content from the browser, add a GitHub token to client
code, or include raw research archives, identity assets, transcripts, secrets,
payment identifiers, or personal data. The reader exposes the production
routes, supported formats, source hierarchy, skills, CTA rules, aggregate
social/funnel metrics, and a bounded Graphify dependency summary. It has no
editing or publishing actions.
