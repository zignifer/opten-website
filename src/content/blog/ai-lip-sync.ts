...
const ru: BlogPostLocale = {
  slug: "ai-lip-sync",
  title: "AI lip sync: синхронизация губ нейросетью",
  excerpt:
    "AI lip sync и синхронизация губ нейросетью работают стабильнее, когда есть чистое аудио, крупный план, stable pose и prompt lock до рендера.",
  description:
  ...
  body: {
    ...
    steps: [
      {
        ...
        body:
          "Липсинк нейросеть начинает ошибаться задолго до рендера, если исходники слабые. Нужны три вещи: чистая дорожка без музыки поверх голоса, лицо хотя бы в среднем плане и видео без резких поворотов головы. Если рот занимает десять пикселей, модель не понимает, какие губы ей синхронизировать.\n\nДля озвучка видео нейросеть workflow сначала отделите голос от фонового шума, затем проверьте дикцию и длину фраз. Слишком быстрая речь дает дерганый рот, слишком длинные паузы заставляют модель «жевать» тишину. Лучше подготовить короткий дубль на 8-15 секунд, проверить его, а потом масштабировать сценарий.",
        before:
      ...
      {
        title: "Синхронизация губ нейросеть: крупный план важнее модели",
        body:
      ...
      },
      {
        ...
        body:
          "Запросы вроде «озвучка видео нейросеть тг» и «озвучка видео нейросеть ютуб» обычно про один и тот же результат: быстро заменить голос и не получить странный рот в финальной публикации. Но площадки смотрятся по-разному. В Telegram часто виден маленький превью-кадр, поэтому ошибки губ менее заметны, зато шум в аудио слышен сразу. На YouTube крупный экран быстрее показывает рассинхрон.\n\nФинальный чек: рот закрывается после фразы, зубы не мерцают, эмоция не прыгает между кадрами, нет лишних субтитров или случайного текста, громкость не скачет. Если один пункт не проходит, исправляйте один параметр за раз: аудио, крупность, позу или timing. Переписывать весь prompt обычно дольше.",
        before:
    ...
    faq: [
      ...
      {
        q: "Почему липсинк нейросеть плохо двигает рот?",
      ...
      {
        q: "Подходит ли озвучка видео нейросеть онлайн для YouTube?",
