...
const ru: BlogPostLocale = {
  ...
  excerpt:
    "Обучение нейросетям и нейросети с нуля проще освоить через задачу, промпт, проверку результата и маленький кейс для портфолио без хаоса из уроков.",
  description:
    "Обучение нейросетям с нуля: какие навыки нужны новичку, как выбирать курсы по нейросетям, писать промпты, проверять ответ и собрать первый кейс без хаоса.",
  category: "guide",
  ...
  body: {
    intro:
      "Обучение нейросетям стоит начинать не с архитектуры моделей, а с практики: выбрать задачу, описать результат, написать промпт, проверить ответ и сохранить кейс за один вечер. Так нейросети с нуля становятся рабочим навыком, а не набором случайных уроков и списков сервисов.",
    steps: [
      {
        title: "Нейросети с нуля - это не сначала математика",
        body:
          "Новичку не нужно начинать с слоев, весов и функции активации, если цель - применять ИИ в работе. Эти темы пригодятся позже, когда вы пойдете в машинное обучение или аналитику. Для маркетолога, дизайнера, предпринимателя, менеджера или фрилансера первый уровень другой: понять, какую задачу можно отдать модели и как оценить результат.\n\nПоэтому обучение нейросетям начинается с маленького результата. Не «разобраться во всем», а «собрать черновик письма клиенту», «сделать структуру презентации», «подготовить описание товара», «сформулировать референс для изображения». Такой подход снижает тревогу: вы учитесь управлять ответом модели, а не сдавать экзамен по нейронным сетям.",
        before:
      ...
      {
        title: "Промпт пишется как короткое ТЗ",
        body:
          "Промпт - это не магическая фраза, а короткое техническое задание. В нем должны быть роль модели, контекст, входные данные, формат ответа и запреты. Если написать «сделай хороший текст», модель даст общий ответ. Если объяснить аудиторию, задачу, тон, объем и ограничения, ответ становится рабочим черновиком.\n\nПрактический кейс: новичок попросил ChatGPT сделать текст для лендинга курса и получил стерильный набор преимуществ. Ошибка была в промпте: не было аудитории, уровня знания, формата блока и запрета на рекламные штампы. После правки добавили роль редактора, контекст курса, аудиторию «специалист, который боится начать с ИИ», формат из 5 блоков и запрет на обещания профессии за неделю. Результат стал похож на черновик страницы, который уже можно редактировать.",
        before:
      ...
      },
      {
        ...
        body:
          "Главный навык после промпта - проверка. Смотрите на факты, тон, формат, полноту и одну главную ошибку. Не переписывайте запрос целиком после каждого ответа: выберите один дефект и исправьте его. Например: «тон слишком рекламный», «нет примеров», «модель не поняла аудиторию», «формат не совпал с заданием».\n\nOpten полезен именно на этапе проверки перед генерацией: он помогает увидеть, где запрос не зафиксировал роль, формат, ограничения или критерий качества, и улучшить промпт. Это экономит попытки и быстрее показывает, какие правила работают для вашей задачи.",
        before:
    ...
    faq: [
      {
        ...
        a: "Первый практический результат можно получить за вечер: выбрать задачу, написать промпт, проверить ответ и исправить одну ошибку. Уверенный рабочий навык обычно появляется после нескольких недель повторения на похожих задачах.",
      },
      {
      ...
      },
      {
        ...
        a: "Да. Бесплатные курсы по нейросетям хороши для первого входа: понять базовые термины и попробовать промпты. Но после вводных уроков переходите к своему кейсу, иначе обучение останется просмотром чужих примеров.",
      },
    ],
  },
};
