...
const ru: BlogPostLocale = {
  ...
  title: "Промпты для Seedance 2.0: видео-сцена без дрейфа",
  excerpt:
    "Промпты для Seedance 2.0 работают лучше, когда сцена разбита на блоки. Разбираем тайминг, движение и ограничения.",
  description:
    "Гайд по промптам для Seedance 2.0: как описывать сцену, движение, объект и ограничения, чтобы видео не теряло бренд и камеру.",
  category: "guide",
  ...
  body: {
    intro:
      "Промпты для Seedance 2.0 — это не список красивых слов, а короткий режиссерский бриф для AI-видео: сцена, объект, движение камеры, тайминг, звук и ограничения. Модель сильна в мультимодальных сценах, но без структуры легко теряет брендовый объект, меняет композицию или смешивает действия.",
    steps: [
      {
        title: "Начните со сцены, а не с эффекта",
        body:
          "Обзор возможностей Seedance 2.0 быстро уводит в звук, референсы и длинные ролики, но рабочий prompt начинается проще: где происходит сцена, что в кадре главное, как камера это видит. Если первым поставить стиль вроде `cinematic, dramatic, high detail`, модель получит настроение, но не получит постановку.\n\nПрактичный порядок: `Scene` -> `Subject` -> `Action` -> `Camera` -> `Lighting` -> `Audio` -> `Constraints`. На syntx.ai и в большинстве API-оберток английский prompt обычно стабильнее, поэтому даже русскую задачу полезно превращать в английский блок. Opten здесь работает как preflight: разворачивает короткую идею в структуру и показывает, где забыты камера, объект или запреты.",
        before:
      ...
      },
      {
        ...
        body:
          "Seedance 2.0 хорошо понимает позитивное описание, но сложные сцены все равно требуют negative constraints. Запрещайте не абстрактное «плохое качество», а конкретные провалы: logo drift, extra products, sudden camera jump, warped hands, plastic skin, unwanted subtitles, random text.\n\nЕсли работаете через API Seedance 2.0 или сервис, где есть отдельное поле negative prompt, вынесите туда артефакты и водяные знаки. Если отдельного поля нет, оставьте `Constraints` в конце основного prompt. Важно не перегружать: 5-8 точных запретов работают лучше, чем длинная стена из всех возможных дефектов.",
        before:
    ...
    faq: [
      {
        q: "Что такое промпты для Seedance 2.0?",
      ...
      {
        q: "API Seedance 2.0 требует другой промпт?",
