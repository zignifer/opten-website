...
const ru: BlogPostLocale = {
  ...
  description:
    "Нейросети для текста: как собрать prompt для поста, письма или лендинга, убрать шаблонный AI-ритм и вручную проверить черновик перед публикацией.",
  category: "guide",
  ...
  body: {
    ...
    steps: [
      ...
      },
      {
        ...
        body:
          "Текст после генерации остается черновиком. Сначала проверьте имена, цифры, ссылки, условия продукта и любые причинно-следственные выводы. Затем прочитайте текст вслух. Повторяющиеся конструкции, одинаковая длина абзацев, лишние выводы и связки вроде «важно отметить» выдают шаблонный AI-ритм. Уберите то, что автор не сказал бы клиенту в обычном разговоре.\n\nПоследний проход сравнивает текст с задачей. Есть ли одно понятное действие? Достаточно ли контекста человеку, который впервые видит продукт? Можно ли сократить начало без потери смысла? Нейросети для контента ускоряют черновик, но ответственность за факты, интонацию и публикацию остается у человека. На фрилансе этот процесс можно продавать как услугу: brief, prompt, редактура и адаптация под канал, а не как безликую «генерацию текстов».",
        ...
        promoBanner: {
          eyebrow: "Курс Opten",
          title: "Превратите AI-черновики в связный контент-проект",
          body:
            "На курсе вы проходите путь от brief и prompt до текстов, визуалов, сайта и рекламных материалов с понятной редакторской проверкой.",
          ...
          image: {
            ...
            width: 1600,
            height: 560,
            alt: "Курс по AI-контенту: workflow от prompt до готового проекта",
          },
        },
      },
    ],
    faq: [
      {
        ...
        a:
          "ChatGPT, Claude, Gemini и другие языковые модели умеют готовить текстовые черновики. Выбирайте не по одной демонстрации, а по реальной задаче: передайте одинаковый brief, сравните точность следования формату, качество русского языка и удобство редактирования.",
      },
      {
        q: "Как написать prompt для текста?",
        a:
