...
const COVER_RU = {
  ...
  width: 1600,
  height: 900,
  alt: "Обложка гайда про замена лица нейросеть и аккуратный AI face swap",
};
const COVER_EN = {
  src: "/blog/ai-face-swap/cover.jpg",
  width: 1600,
...
const ru: BlogPostLocale = {
  slug: "ai-face-swap",
  title: "Замена лица нейросеть: аккуратный face swap без артефактов",
  excerpt:
    "Замена лица нейросеть работает чище, когда исходники совпадают по ракурсу, свету и мимике. Разбираем нейросеть замена лица онлайн без грязных краев.",
  description:
    "Замена лица нейросеть: как подобрать исходники, описать свет, кожу и мимику, чтобы замена лица фото нейросеть онлайн не дала артефакты.",
  category: "guide",
  ...
  body: {
    intro:
      "Замена лица нейросеть — это image-editing задача, где модель переносит идентичность из референса в целевой кадр. Чистый результат зависит не от кнопки «swap», а от исходников: ракурс, свет, кожа и мимика должны быть описаны явно, иначе нейросеть замена лица онлайн оставит артефакты.",
    steps: [
      {
        title: "Подберите исходники для замены лица нейросеть онлайн",
        body:
          "Хороший AI face swap начинается до промпта. Нужны два кадра: референс лица и целевая сцена. Чем ближе поворот головы, фокусное расстояние, выражение и свет, тем меньше модель будет «додумывать». Если референс снят фронтально, а целевой кадр в три четверти и с жестким боковым светом, замена лица онлайн нейросеть почти наверняка даст чужую челюсть, пластиковую кожу или странные края у волос.\n\nРабочее правило простое: используйте свои, согласованные или полностью синтетические лица; не берите знаменитостей и частных людей без разрешения. Для фото лучше держать оба изображения в близком разрешении и не обрезать уши, подбородок и линию волос. Для видео добавьте первый кадр сцены как отдельный reference, иначе лицо может совпасть в начале и поплыть на движении.",
        before:
    ...
    faq: [
      {
        q: "Как работает замена лица нейросеть?",
      ...
      {
        q: "Где лучше делать замена лица нейросеть онлайн?",
      ...
      {
        q: "Почему замена лица фото нейросеть онлайн дает артефакты?",
