...
const ru: BlogPostLocale = {
  ...
  title: "Логотип через нейросеть: промпт для бренда",
  excerpt:
  ...
  description:
    "Как сделать логотип через нейросеть: написать промпт для бренда, выбрать стиль, задать ограничения и исправить первый результат без лишних деталей.",
  category: "guide",
  ...
  body: {
    intro:
      "Логотип через нейросеть можно быстро довести до формы знака, но хороший результат начинается с промпта: кто бренд, где будет использоваться знак, какой стиль нужен и какие детали запрещены. Для логотипа важнее простота, читаемость в малом размере и ограничения, чем длинный список красивых эффектов.",
    steps: [
      {
        title: "Сформулируйте бриф до первого промпта",
        body:
          "Генератор логотипов на ИИ не понимает бизнес-контекст сам по себе. В первом блоке промпта задайте сферу бренда, аудиторию, характер и носители: сайт, иконка приложения, упаковка, аватарка, презентация. Не просите сразу «полный брендбук» или «идеальный логотип» - это слишком широкая задача. Лучше описать один знак для одного сценария. Например: «минималистичный знак для сервиса бронирования студий, должен читаться как app icon и favicon». Opten в таком сценарии полезен как prompt checker: он быстро показывает, где запрос звучит как настроение, а где уже стал рабочим брифом.",
        before:
    ...
    faq: [
      ...
      {
        q: "Почему ИИ-генератор логотипов делает слишком сложный знак?",
