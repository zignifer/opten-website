...
const ru: BlogPostLocale = {
  ...
  body: {
    intro:
      "Курсы по ИИ стоит выбирать по практическому результату, а не по обещаниям профессии за неделю. Хорошая программа показывает, как поставить задачу модели, написать промпт, проверить ответ и собрать портфолио-кейс. Если курс по нейросетям не ведет к артефакту, новичок получает конспект вместо навыка.",
    steps: [
      {
        title: "Курсы по ИИ для начинающих: начните с результата",
        body:
          "Перед оплатой курса сформулируйте один результат, который вам нужен через 7-14 дней: презентация, лендинг, карточка товара, видео, серия постов или повторяемый процесс работы с промптами. Если программа обещает «все про ИИ», но не показывает, что именно вы сделаете руками, новичку будет трудно понять прогресс.\n\nПрактический кейс: специалист выбирал между большим курсом «профессия ИИ-эксперт» и короткими уроками Opten. Вместо покупки всего пакета он выбрал один результат: лендинг для тестового продукта. Первый промпт в ChatGPT был слабым: «сделай структуру лендинга для курса». Исправление: добавили аудиторию, продукт, формат блоков, запрет на обещания заработка и критерий качества. Через вечер у него был черновик страницы, а не просто список инструментов.",
        before:
          "Хочу пройти курсы по ИИ. Посоветуй, что выбрать, чтобы быстро разобраться во всем.",
        after:
          "Цель обучения: за неделю собрать один лендинг для тестового продукта. Нужны структура страницы, тексты блоков, визуальное задание и список правок. Критерий: кейс можно показать как черновик портфолио.",
        imageSrc: "/blog/ai-courses-for-beginners/ru/step-1.jpg",
      },
      {
        title: "Красные флаги курсов по нейросетям",
        body:
          "Сильные курсы по нейросетям не продают гарантию трудоустройства, сертификат вместо навыка или «профессию за неделю». Они честно объясняют, какие задачи вы научитесь делать, где нужна практика и какие ограничения есть у моделей. Красный флаг - программа из длинных обзорных уроков без домашнего задания, разбора и финального проекта.\n\nНе ищите «лучшие курсы по нейросетям» как рейтинг школ без контекста. Для новичка лучший вариант зависит от задачи: работа с текстом, визуал, презентации, видео, маркетплейсы или сайт. Сравнивайте не громкость обещаний, а структуру: сколько практики, как проверяют результат, есть ли повторяемый рабочий процесс и можно ли закончить мини-кейс.",
        before:
      ...
      },
      {
        ...
        body:
          "В хорошем курсе по ИИ и нейросетям есть не только список сервисов. Нужны блоки про постановку задачи, контекст, промпт, ограничения, проверку ответа, итерацию и упаковку результата. Модель может быстро дать черновик, но качество появляется там, где ученик умеет объяснить цель, аудиторию, формат и критерий проверки.\n\nМинимальный набор практики: один текстовый кейс, одно визуальное задание, одна презентация или лендинг, одно упражнение на исправление слабого промпта и финальная упаковка результата. Opten здесь полезен для проверки перед генерацией: он помогает заметить, что в запросе не хватает роли, формата, ограничений или критерия качества до того, как вы тратите попытки.",
        before:
          "Программа: обзор ChatGPT, обзор генераторов картинок, обзор видео, обзор автоматизаций.",
        after:
          "Программа: задача, промпт, первая ошибка, правка, проверка результата, финальный кейс и короткое описание для портфолио.",
        imageSrc: "/blog/ai-courses-for-beginners/ru/step-3.jpg",
      },
      {
        title: "Портфолио важнее красивого сертификата",
        body:
          "Сертификат помогает только как подтверждение участия. Для работы, фриланса или внутренней задачи важнее показать артефакт: исходную задачу, промпт, первую ошибку, исправление и финальный результат. Так видно, что вы не просто смотрели уроки, а умеете доводить задачу для ИИ до результата.\n\nЕсли бюджет ограничен, начните с бесплатных уроков Opten в разделе Learn: выберите один сценарий и соберите мини-кейс. Платные курсы по ИИ для начинающих имеют смысл, когда вы уже понимаете, какой результат хотите повторять и где вам не хватает обратной связи, дисциплины или глубины.",
        before:
          "После курса у меня есть конспект, сертификат и список сервисов, но нет результата, который можно показать.",
        after:
          "После обучения у меня есть кейс: задача, промпт, ошибка, исправление, финальный артефакт и выводы для следующей работы.",
        imageSrc: "/blog/ai-courses-for-beginners/ru/step-4.jpg",
        promoBanner: {
          eyebrow: "Курс Opten",
          title: "Соберите практику с ИИ вокруг одного проекта",
          body:
            "Курс показывает полный цикл: промпт, изображения, видео, сайт и рекламные материалы для одного проекта, чтобы обучение сразу превращалось в портфолио-кейс.",
    ...
    faq: [
      {
        ...
        a: "Выбирайте курсы по ИИ для начинающих, где есть практические задания, обратная связь и финальный артефакт: презентация, лендинг, визуал, видео или повторяемый процесс работы с промптами. Если программа только перечисляет сервисы, навык закрепляется хуже.",
      },
      {
        q: "Нужен ли сертификат после курса по нейросетям?",
        a: "Сертификат может подтвердить участие, но сам по себе редко показывает навык. Сильнее работает портфолио-кейс: задача, исходный промпт, ошибка, исправление и финальный результат.",
      },
      {
