...
const ru: BlogPostLocale = {
  ...
  excerpt:
    "Обучение ИИ и обучение нейросетям быстрее дают результат, когда новичок выбирает одну задачу, пишет промпт, сравнивает ответ и улучшает его.",
  description:
    "Обучение ИИ с нуля: как выбрать первую задачу, построить семидневную практику, писать промпты, перейти от курсов по ИИ к портфолио и обучению нейросетям.",
  category: "guide",
  ...
  body: {
    intro:
      "Обучение ИИ с нуля стоит начинать не со списка сервисов, а с одной практической задачи. Новичку нужно выбрать формат результата, написать промпт как короткое ТЗ, сравнить ответы модели и улучшить запрос. Так обучение нейросетям быстрее превращается в навык, а не в бесконечный обзор инструментов.",
    steps: [
      {
      ...
      },
      {
        ...
        body:
          "Нейросети с нуля проще осваивать через одну из трех траекторий. Первая - текст: письма, посты, описания товаров, резюме встреч. Вторая - презентации: структура, тезисы, слайды, сценарий выступления. Третья - визуал и видео: референсы, обложки, рекламные кадры, короткие ролики. Не смешивайте их в первую неделю.\n\nЕсли вам нужны курсы по ИИ, выбирайте те, где есть повторяемый рабочий процесс, а не только обзор сервисов. Хороший учебный модуль заставляет сделать артефакт: слайд, лендинг, карточку, видео, мини-проект. Без артефакта обучение остается просмотром чужого экрана.",
        before:
          "Пойду изучать ChatGPT, Midjourney, Veo, Figma AI, автоматизации и еще пару сервисов.",
        after:
          "Траектория на неделю: только презентации. День 1 - задача и аудитория. День 2 - структура. День 3 - промпт для слайдов. День 4 - визуальный стиль. День 5 - правки. День 6 - сборка. День 7 - один готовый кейс.",
        imageSrc: "/blog/ai-training-beginners/ru/step-2.jpg",
      },
      {
        title: "Промпт - это ТЗ для нейросети",
        body:
          "Промпт работает как техническое задание: он должен объяснить цель, контекст, входные данные, формат ответа и запреты. Если написать «сделай красиво», модель угадает стиль сама. Если указать аудиторию, роль, тон, формат и ограничения, результат становится проверяемым.\n\nПрактический кейс: новичок делал карточку товара в GPT Image 2 и получил красивую, но бесполезную картинку. Ошибка была в промпте: он просил «рекламное фото товара», но не фиксировал аудиторию, ракурс, фон и место под текст. Исправление: добавили товар в центре кадра, покупателей подарков как аудиторию, темный нейтральный фон, свободное место под заголовок слева сверху и запрет на выдуманные логотипы и нечитаемый текст. Следующий рендер стал пригоден для портфолио: понятный товар, чистый фон и место под заголовок.",
        before:
          "Сделай рекламную карточку товара, красиво, современно, для маркетплейса.",
        after:
          "Рекламная карточка керамической кружки ручной работы. Аудитория: покупатели подарков. Темный нейтральный фон, мягкий студийный свет, кружка в центре, свободное место под заголовок слева сверху, реалистичная фактура. Не добавляй выдуманные логотипы, нечитаемый текст и другие товары.",
        imageSrc: "/blog/ai-training-beginners/ru/step-3.jpg",
      },
      {
        title: "Мини-план на 7 дней: от запроса до портфолио",
        body:
          "День 1: выберите одну задачу и запишите критерий качества. День 2: соберите 3 референса. День 3: напишите первый промпт. День 4: сравните 2-3 результата и найдите одну главную ошибку. День 5: перепишите промпт, не меняя всю задачу. День 6: доведите результат до публикабельного вида. День 7: сохраните кейс с исходным промптом, исправлением и финалом.\n\nOpten полезен на этапе проверки перед генерацией: он помогает превратить русскую идею в готовый промпт, подсветить пропущенные ограничения и улучшить запрос. Сначала учитесь ставить задачу модели, потом уже расширяйте набор инструментов.",
        before:
          "Каждый день смотреть новый урок и пробовать новый сервис.",
        after:
          "Каждый день двигать один кейс: задача, референсы, промпт, первая ошибка, исправление, финальный результат, короткое описание для портфолио.",
        imageSrc: "/blog/ai-training-beginners/ru/step-4.jpg",
        promoBanner: {
          eyebrow: "Курс Opten",
          title: "Соберите практику на нейросетях в один процесс",
          body:
    ...
    faq: [
      {
        ...
        a: "Начните с одной маленькой задачи, которую можно закончить за вечер: текст, слайд, карточка товара, обложка или короткий сценарий. Затем напишите промпт, получите результат, найдите главную ошибку и улучшите запрос.",
      },
      {
      ...
      },
      {
        ...
        a: "Да, первые шаги можно делать бесплатно: выбрать задачу, писать промпты, сравнивать ответы и фиксировать ошибки. Платные инструменты нужны позже, когда вы понимаете, какой результат хотите получить и зачем.",
      },
      {
        q: "Чем обучение ИИ отличается от списка инструментов?",
        a: "Список инструментов отвечает на вопрос «где нажать». Обучение ИИ отвечает на вопрос «как поставить задачу модели, проверить результат и улучшить промпт». Второй навык переносится между сервисами.",
      },
    ],
  },
};
