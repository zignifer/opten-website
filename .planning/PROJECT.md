...
`opten.space` is the public website for **Opten**, a Chrome extension that scores and improves AI image-generation prompts inline in supported AI generators. The extension remains the primary shipped product, but the site now has six durable jobs: marketing surface (RU/EN landing), website-first auth/billing (`/login`, `/pay`, `/account`, `/success`), Pro-only utilities (`/dashboard/download-skill`), Prompt Library management/public snapshots, public Learn SEO pages, and hidden/noindex Kinescope course funnels including the Telegram hidden intro lead magnet.
...
**Idle between formal GSD milestones.** Since v2.0, the site shipped several ad-hoc/product surfaces: website Email OTP auth, Prompt Library public snapshots, public Learn/Learn Finds, a hidden Kinescope course, and the Telegram hidden intro funnel with a 24h personal discount claim.

Current active product thread: build a protected general owner admin surface for traffic/acquisition channels. First use-case is Telegram hidden intro stats and broadcasts, but the route should be designed as `/admin/*`, not as a Telegram-only panel.

Active artefacts: [ROADMAP.md](ROADMAP.md) · [STATE.md](STATE.md). (No `REQUIREMENTS.md` — fresh one is created per milestone; v2.0's is archived.)

### Shipped previously

- **v2.0 — Programmatic SEO — Model Pages — SHIPPED 2026-05-20.** 62 model pages × RU/EN + 2 hubs = 144 prerendered routes (126 model HTML). 62 content files generated by 7 parallel agents, verified 62/62, IndexNow pinged. Hotfix series: EN meta + head-meta localization, hub redesign to blog parity, blog/models hydration fix, seedance dedupe, 11 umbrella cards hidden (kept live for generic-query SEO). Archive: [milestones/v2.0-ROADMAP.md](milestones/v2.0-ROADMAP.md) · [milestones/v2.0-REQUIREMENTS.md](milestones/v2.0-REQUIREMENTS.md).
- **Post-v2.0 product surfaces — SHIPPED 2026-06..2026-07.** Website Email OTP auth is canonical for site surfaces; `/pay` and `/account` prefer the website JWT and keep extension fallback. Prompt Library public snapshots live at `/p/:slug`. Public Learn and Learn Finds are indexed RU/EN. Hidden Kinescope course routes live under `/learn/courses/*`. Telegram hidden intro unlocks lesson 0 after channel subscription, records funnel events, reports lesson opens, and applies a 24h `course_discount_claims` discount with priority over manual promo codes.
...
- **State:** the site owns no primary database. Browser state lives in `localStorage` (`opten_lang_v3`, website session, course/lesson hints) while durable users, subscriptions, course orders/entitlements, Prompt Library rows, Telegram leads/events, and discount claims live in the extension-owned self-hosted Supabase backend.
...
---
*Last updated: 2026-07-07 after Telegram hidden intro + 24h discount claim context refresh. Next planned work: protected general owner admin surface.*
