...
- [x] **Phase 4.2: SEO/GEO polish (post-synthesis)** *(INSERTED 2026-05-17, completed 2026-05-17)* — closed the P0 + P1 actionable findings from the dual Codex CLI review. 7 atomic wave commits (5 autonomous + 2 user-approved: W6 visible h1 on /pay+/welcome, W7 X-Robots-Tag headers in vercel.json) + 1 code-review fix commit. 4 user decisions captured (D-1 definitional paragraph stays in footer, D-2 canonical model count = 60+, D-3 CSP punted to separate ticket, D-4 aggregateRating removed until honest reviews accumulate). Founder image (P1-6) deferred — needs user-supplied WebP. Architectural refactor deferred to Phase 6. Verification 14/14 ROADMAP success criteria PASS at code/build level; 5 post-deploy / human-only checks (Vercel edge X-Robots materialization, visual h1 smoke, Bing token replacement, IndexNow acceptance 24-72h, GEO score rescore window 7-14 days) tracked in `phases/04.2-seo-geo-polish-post-synthesis/04.2-VERIFICATION.md`.
- [x] **Phase 5: Brand authority** — **CLOSED AS DEFERRED TO v2 MILESTONE (2026-05-17)** — entirely off-site marketing work (Product Hunt launch, Wikipedia/Wikidata entry, Reddit + Habr + YouTube seeding, expanded `sameAs` schema after authority URLs acquired). Not auto-executable in a code milestone. Scope preserved in Phase Details below for v2 picking. The single code component (sameAs additions to Organization JSON-LD) folds into v2 Phase 1 when authority URLs are real.
- [x] **Phase 6: Scale-ready content architecture** *(added 2026-05-17, closed 2026-05-17)* — **CLOSED AS DEFERRED TO v2 MILESTONE** — requires content strategy decision upstream (which archetypes, how many pages, in what locales) before a refactor of route inventory + schema templating makes sense. Scope sketch preserved in Phase Details below for v2 picking up once content strategy is set. The current 16-route hand-listed inventory is fine for the v1 milestone.
...
| 4.2. SEO/GEO polish (post-synthesis) | 7/7 + 1 code-review fix | Complete (14/14 ROADMAP criteria PASS code/build; 5 post-deploy checks deferred) | 2026-05-17 |
| 5. Brand authority | — | Closed as deferred to v2 (off-site, not code-trackable) | 2026-05-17 |
| 6. Scale-ready content architecture | — | Closed as deferred to v2 (needs content strategy upstream) | 2026-05-17 |

---
*Roadmap defined: 2026-05-14*
*Last updated: 2026-05-17 — v1.0 milestone CLOSED. Phase 4.2 shipped (7 waves + 1 code-review fix + REVIEW + VERIFICATION, deployed to opten.space). Phase 5 (brand authority) and Phase 6 (scale-ready architecture) closed as deferred to v2 milestone — off-site marketing work and content-strategy-dependent refactor, respectively. Composite GEO Score trajectory: 12 (Phase 1 baseline 2026-05-14) → ~30 (after Phase 1) → 48 (post-Phase 3) → 63 (2026-05-17 morning baseline, internal audit) → ~72.6 (post 10 morning fixes + dual Codex review) → target ~80+ after Phase 4.2 deploy lands and IndexNow / Bing ping cycles complete (7-14 days).*
