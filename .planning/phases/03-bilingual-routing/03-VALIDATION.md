...
| 07 (sitemap.mjs lists 12 entries with hreflang annotations) | 3 | GEO-C-2, GEO-C-3 | — | `dist/sitemap.xml` lists every shipped page (6 RU + 6 EN); each entry carries `<xhtml:link rel="alternate" hreflang="...">` annotations for its siblings | source assert + xmllint or grep | `npm run build && grep -c '<loc>' dist/sitemap.xml` — 12; `grep -c '<xhtml:link' dist/sitemap.xml` — ≥ 24 (12 entries × 2 alternates each) | ❌ Plan 07 emits | ⬜ pending |
