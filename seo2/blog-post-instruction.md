...
крупный короткий текст только на внутристатейных картинках.
...
4. Проверь, что RU/EN внутристатейные изображения уже содержат нужный текст в
   самом растре и не требуют отдельного текстового слоя.
5. Проверь, что prerendered RU/EN HTML содержит:
   - correct `lang`;
   - canonical;
   - reciprocal hreflang;
   - BlogPosting;
   ...
   - рабочие image refs.
6. Запусти `git status --short` и отдели свои изменения от уже существующих
