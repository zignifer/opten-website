...
- Keyword lock — это semantic coverage, а не дословная вставка кривой поисковой
  строки. В видимом тексте, title, description, intro, FAQ и примерах нельзя
...
- Для всех новых SEO2-статей не заполняй поля шагов `before` и `after`, не
  создавай видимые блоки «Было / Стало» или `Before / After` и не имитируй ту
  же пару другими заголовками. Полезный итоговый пример показывай отдельно:
  как готовый промпт, чек-лист, последовательность действий, аннотированный
  результат или разбор решения. Уже опубликованные статьи не переделывай.
...
композиционных идей**: split-screen без пары «Было / Стало», moodboard,
timeline, крупный объект, одна сцена с callout, последовательность кадров. Не прикладывай
...
- `- Humanized frames: 1` для стандартной серии;
- style continuity: какие признаки должны остаться одинаковыми при смене
  композиции.

### Живое присутствие без случайных стоковых людей

В стандартной серии один inline-кадр должен содержать осмысленное человеческое
присутствие, а остальные кадры должны доказывать тезисы через предметы, среду и
действие без людей. Обложка в эту квоту не входит. Цель — убрать стерильность
серии, но не превращать статью в четыре одинаковых кадра с персонажами.

В production sheet укажи для каждого `## Frame N` одно точное поле:

- `- Human presence: none`
- `- Human presence: hands-only`
- `- Human presence: human-detail`
- `- Human presence: single-person`
- `- Human presence: interaction`

Для каждого значения кроме `none` добавь `- Human purpose:` и объясни, какое
действие, решение, масштаб, проверку, передачу результата или взаимодействие
человек делает видимым. Человек не может быть декоративным работником за
ноутбуком, случайной толпой или стоковым офисным фоном. Предпочтительный порядок
выбора: руки, которые реально работают с главным объектом; выразительная деталь
жеста или позы; один герой в точке решения; осмысленное взаимодействие людей,
если тезис действительно про клиента, команду или сообщество.

По умолчанию `- Humanized frames: 1`. Для темы, где человек является самим
предметом статьи — например, аватары, портреты, UGC или постоянство персонажа —
можно использовать два humanized-кадра, но тогда production sheet должен
содержать непустое `- Human-heavy exception:` с причиной. Больше двух нельзя,
и хотя бы один inline-кадр всегда должен остаться без людей.

Если `Human presence: none`, в generation prompt явно пиши `no people, no
hands, no human silhouettes`, чтобы модель не добавляла случайную фигуру. Если
человек присутствует, prompt должен зафиксировать количество людей, роль,
действие, ракурс и кадрирование. В EN localization-edit сохраняй того же
человека, позу, действие, одежду, кадрирование и сцену; меняй только текст.
...
- visual proof: какой объект, контраст, действие или результат показывает эту
  мысль без текста;
- composition archetype: split-screen без сравнения «Было / Стало», sequence,
  hero visual, moodboard, anatomy, timeline, decision point и так далее;
- text mode:
...
- human presence: `none`, `hands-only`, `human-detail`, `single-person` или
  `interaction`;
- human purpose: для humanized-кадра — зачем человек нужен для доказательства
  тезиса, а не только для атмосферы;
...
Исключение: рекламные promo CTA banners внутри блога. Это не SEO2 inline
explainer image, а reusable баннер для нужного продукта. Делай широкий низкий растр
...
вшивать в изображение; его рендерит сайт. Основной баннер ведёт на
`primaryDestination` выбранной темы: `/`, `/prompt-library`, `/learn` или
`/learn/courses/ai-content-marketing-2026`. Не подменяй этот intent другим
продуктом только потому, что для него уже есть готовый баннер.
...
7. Открой все RU inline-кадры рядом и проверь серию:
   ...
   - стандартная серия содержит ровно один humanized inline-кадр; второй
     допустим только с `Human-heavy exception`, людей нет во всех кадрах сразу;
   - humanized-кадр показывает действие/решение/масштаб/проверку/передачу, а не
     случайного человека для атмосферы;
   - кадры с `Human presence: none` действительно не содержат людей, рук или
     силуэтов.
   Затем открой все EN-пары и проверь, что меняется только язык, а не идея,
   камера, композиция, человек, поза или действие.
