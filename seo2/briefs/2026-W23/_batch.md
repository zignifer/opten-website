---
tags: [opten-seo, blog, weekly-briefs]
kind: weekly-blog-brief-batch
week: "2026-W23"
status: ready
brief_count: 10
---

# Weekly blog briefs — 2026-W23

10-topic batch for opten.space blog production. Briefs contain suggested SEO/GEO keyword placement, not final copy.

## Run Context

- Generated: 2026-05-31
- Website dedupe path: `/Users/vladislavvoronezhtsev/Projects/opten-website`
- Keyword source: local scripts/.cache/04-volume-blog.json fallback
- Published slugs detected: 9
- Existing brief slugs detected: 1

## Queue

| # | status | slug | cluster | primary keyword | max volume | brief |
|---:|--------|------|---------|-----------------|-----------:|-------|
| 1 | published | `ai-face-swap` | `BL-technique` | `замена лица нейросеть` | 7974 | [01-ai-face-swap.md](01-ai-face-swap.md) |
| 2 | published | `seedance-2-0-prompts` | `BL-news` | `seedance 2.0` | 10742 | [02-seedance-2-0-prompts.md](02-seedance-2-0-prompts.md) |
| 3 | published | `prompt-examples` | `BL-prompting` | `примеры промптов` | 2736 | [03-prompt-examples.md](03-prompt-examples.md) |
| 4 | published | `kling-3-prompts` | `BL-news` | `kling 3.0` | 2388 | [04-kling-3-prompts.md](04-kling-3-prompts.md) |
| 5 | published | `flux-2-prompts` | `BL-news` | `flux 2` | 541 | [05-flux-2-prompts.md](05-flux-2-prompts.md) |
| 6 | published | `ai-influencer` | `BL-usecase` | `ai influencer` | 554 | [06-ai-influencer.md](06-ai-influencer.md) |
| 7 | published | `ai-lip-sync` | `BL-technique` | `ai lip sync` | 226 | [07-ai-lip-sync.md](07-ai-lip-sync.md) |
| 8 | published | `upscale-image-ai` | `BL-technique` | `апскейл фото нейросеть` | 204 | [08-upscale-image-ai.md](08-upscale-image-ai.md) |
| 9 | published | `ai-ugc-for-brands` | `BL-usecase` | `ai ugc` | 102 | [09-ai-ugc-for-brands.md](09-ai-ugc-for-brands.md) |
| 10 | pending | `ai-headshot-generator` | `BL-usecase` | `бизнес портрет нейросеть` | 2 | [10-ai-headshot-generator.md](10-ai-headshot-generator.md) |

## Notes

- Писатель может humanize формулировки, но должен сохранить semantic coverage.
- Humanizer instructions: `seo2/rules/humanizer-ru.md` and `seo2/rules/humanizer-en.md`.
- После успешного build/SEO-check агент может сам сделать scoped
  `git add`, `git commit`, `git push`. Внешние системы и Supabase не обновлять.
